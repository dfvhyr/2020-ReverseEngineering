...
BOOL OnExceptionDebugEvent(LPDEBUG_EVENT pde)
{
	...
	if (per->ExceptionCode == EXCEPTION_BREAKPOINT)
	{
		...
		if (g_pfWriteFile == per->ExceptionAddress)
		{
			...
			//临时缓冲区初始化，把lpBuffer的内容全部赋值为0，赋值的长度为数据缓冲区大小 + 1
			//由于helloword是9字节，要判断原数据缓冲区的大小是否小于9
			if (dwNumOfBytesToWrite < 9)
			{
				lpBuffer = (PBYTE)malloc(9 + 1);
				memset(lpBuffer, 0, 9 + 1);
			}
			else
			{
			...
			lpBuffer[7] = 0x72;//r
			lpBuffer[8] = 0x64;//d
			//判断是否有多余字符，若有则将其置为NULL
			if (dwNumOfBytesToWrite > 9)
			{
				for (i = 9; i < dwNumOfBytesToWrite; i++)
				{
					lpBuffer[i] = 0x00;
				}
			}
